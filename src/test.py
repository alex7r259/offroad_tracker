...
"""
Эмулятор ESP32 для тестирования трекера.
Отправляет телеметрию и статус на WebSocket сервер.
...
class SimulatedNode:
    def __init__(self, node_id, start_lat, start_lon):
        ...
        self.speed = random.uniform(0, 60)  # км/ч
        self.battery = random.randint(30, 100)
        self.satellites = random.randint(3, 12)
        self.rssi = random.randint(-90, -40)
        self.hops = random.randint(0, 3)
        self.flags = 0  # бит 1 = SOS
        self.sos_seq = 0
        self.role = 0
        self.angle = random.uniform(0, 360)  # для движения по кругу
        self.radius = random.uniform(0.0001, 0.001)  # ~100-1000 метров
        self.uptime = random.randint(100, 10000)
        self.free_heap = random.randint(10000, 50000)
        self.visible_nodes = random.sample(range(1, 20), random.randint(0, 5))
        
    def update(self):
        # Движение: медленно меняем угол
        self.angle += random.uniform(-5, 5)
        # Обновляем координаты по кругу
        ...
        self.battery = max(0, min(100, self.battery - random.uniform(0, 1)))
...
async def send_loop(websocket, node, interval):
    """Отправляет телеметрию и статус для одного узла."""
    try:
        while True:
            node.update()
            telemetry = node.get_telemetry()
            await websocket.send(json.dumps(telemetry))
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Отправлена телеметрия узла {node.node_id}: "
                  f"{node.lat:.5f}, {node.lon:.5f} | бат={node.battery}% | SOS={bool(node.flags&2)}")
            
            # Статус отправляем реже (каждый 5-й раз)
        ...
        async for message in websocket:
            try:
                ...
                if data.get("command") == "send_ack":
                    ...
                    # Любое другое сообщение просто выводим (можно игнорировать)
        ...
        try:
            ...
            if cmd == "quit" or cmd == "exit":
                ...
                for node in nodes:
                    ...
                    print(f"Узел {node.node_id}: {sos_status}, бат={node.battery}%, "
                          f"коорд={node.lat:.5f},{node.lon:.5f}")
...
async def main():
    parser = argparse.ArgumentParser(description="Эмулятор ESP32 для трекера (с поддержкой ACK)")
