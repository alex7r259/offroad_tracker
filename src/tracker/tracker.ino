...
bool isDuplicate(uint16_t sourceNodeId, uint32_t sequence) {
  ...
  seenPackets[slot].nodeId = sourceNodeId;
  seenPackets[slot].sequence = sequence;
  seenPackets[slot].seenAt = now;
  return false;
}
...
bool shouldRelayPacket(const TrackerPacket &packet, int16_t rssi) {
  ...
  return packet.hops < RELAY_ALWAYS_UNTIL_HOPS ||
         rssiUnknown ||
         rssi < RELAY_WEAK_RSSI_DBM;
}
  ...
  positionIntervalMs =
    static_cast<uint32_t>(
        max(
            ...
            static_cast<uint32_t>(server.arg("interval").toInt())
        )
    );
